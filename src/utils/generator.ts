...
// ─────────────────────────────────────────────────────────────
// Auto-generate pickup loads that force multi-vehicle distribution
//
// Goal: total load should be ~75–90% of total fleet capacity so that
// no single vehicle can carry everything, but the problem is feasible.
//
// Strategy:
//   - Target total load = floor(totalFleetCap * 0.80)
//   - Each pickup gets load = floor(perVehicleCap * 0.55) ± small random
//   - This ensures each vehicle fills up after ~1-2 pickups, distributing work
//
// Returns a comma-separated string ready for the rawPickupLoad field.
// ─────────────────────────────────────────────────────────────
export function generatePickupLoads(
    numPickups: number,
    vehicleCapacity: number | number[],
    numVehicles: number
): string {
    const perVehicleCap = Array.isArray(vehicleCapacity)
        ? Math.floor(vehicleCapacity.reduce((s, v) => s + v, 0) / numVehicles)
        : vehicleCapacity;

    const totalFleetCap = Array.isArray(vehicleCapacity)
        ? vehicleCapacity.reduce((s, v) => s + v, 0)
        : vehicleCapacity * numVehicles;

    // Target 80% of fleet capacity spread across all pickups
    const targetTotal = Math.floor(totalFleetCap * 0.80);

    // Base load per pickup — aim so that ~55% of per-vehicle cap fills one stop
    // This means a vehicle needs ~2 pickups before hitting capacity
    const baseSingleLoad = Math.max(1, Math.floor(perVehicleCap * 0.55));

    const loads: number[] = [];
    let remaining = targetTotal;

    for (let i = 0; i < numPickups; i++) {
        const isLast = i === numPickups - 1;

        if (isLast) {
            // Last pickup gets whatever is left (clamped to reasonable range)
            const last = Math.max(1, Math.min(remaining, perVehicleCap));
            loads.push(last);
        } else {
            // Add slight randomness: baseSingleLoad ± 1
            const jitter = Math.random() < 0.5 ? -1 : 1;
            const load = Math.max(1, Math.min(baseSingleLoad + jitter, perVehicleCap));
            loads.push(load);
            remaining -= load;

            // If remaining is already very low, cap the rest at 1
            if (remaining <= 0) {
                for (let j = i + 1; j < numPickups; j++) {
                    loads.push(1);
                }
                break;
            }
...
export function generateInstance(params: GenerateParams): GenerateResult {
    ...
    // ── 2. Parse pickup load ───────────────────────────────────
    // If rawPickupLoad is the flat all-1s default OR doesn't match numPickups,
    // auto-generate smart loads that force multi-vehicle distribution.
    const capacityForGen = capResult.error ? 10 : capResult.value;
    const autoGenerated = generatePickupLoads(numPickups, capacityForGen, numVehicles);

    const rawToUse = (() => {
        const trimmed = rawPickupLoad.trim();
        if (!trimmed) return autoGenerated;

        // Check if it matches numPickups — if not, fall back to auto
        const parts = trimmed.split(",").map((s) => s.trim());
        if (parts.length !== numPickups) return autoGenerated;

        // Check if it's the boring all-1s default — replace with smart loads
        const allOnes = parts.every((p) => p === "1");
        if (allOnes) return autoGenerated;

        return trimmed;
    })();

    const loadResult = parsePickupLoad(rawToUse, numPickups);
